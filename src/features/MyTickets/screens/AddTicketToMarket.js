...
import { getDayNumber, getMonthAbbr, AddingTicketToMarketPlace } from '../services/api'
import { ScrollView } from 'react-native-gesture-handler'
import NavbarPrimary from '../../../components/NavbarPrimary'

const AddTicketToMarket = (props) => {
  ...
  async function handleSubmit (e) {
    ...
    if (!error) {
      ...
      try {
        ...
        navigation.navigate('Agregar ticket al market ok')
      } catch (error) {
  ...
  return (
    <View className="bg-secondary flex-1">
        <NavbarPrimary title='Publicar ticket en marketplace'/>
        ...
        <View className="pt-6 mt-28 pb-10">
          <View className="pb-4 pt-4" >
                ...
                <TextInput
                placeholder={`Min 1 ARS - Max ${ticket.price * 5}`}
          ...
          {/* <ButtonPrimary title="PUBLICAR" onPress={() => { navigation.navigate('Agregar ticket al market ok') }}/> */}
          <ButtonPrimary title="PUBLICAR" onPress={e => handleSubmit(e)}/>
        </View>
        </ScrollView>
    </View>
  )
...
export default AddTicketToMarket
